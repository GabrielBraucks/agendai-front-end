...
class _EmployeesState extends State<Employees> {
  ...
  void _showAddEmployeeDialog() {
    ...
    final passwordController =
        TextEditingController(); // Added for new employee
    ...
    showDialog(
      ...
      builder: (BuildContext dialogContext) {
        final presenter =
            Provider.of<EmployeesPresenter>(dialogContext, listen: false);
        return AlertDialog(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  TextFormField(
                      ...
                      validator: (v) =>
                          v!.isEmpty ? 'CPF é obrigatório' : null),
                  const SizedBox(height: 12),
                  TextFormField(
                      ...
                      validator: (v) =>
                          v!.isEmpty ? 'Cargo é obrigatório' : null),
                  const SizedBox(height: 12),
                  TextFormField(
                  ...
                  TextFormField(
                      ...
                      validator: (v) =>
                          v!.isEmpty ? 'Telefone é obrigatório' : null),
                  const SizedBox(height: 12),
                  TextFormField(
                ...
                if (formKey.currentState!.validate()) {
                  try {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                        content: Text('Funcionário adicionado!'),
                        backgroundColor: Colors.green));
                  } catch (e) {
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                        content: Text('Falha: ${e.toString()}'),
  ...
  void _showEditEmployeeDialog(Employee employeeToEdit) {
    ...
    final phoneController =
        TextEditingController(text: employeeToEdit.telefone);
    final dataNascController =
        TextEditingController(text: employeeToEdit.dataNasc);
              ...
              onPressed: () async {
                if (formKey.currentState!.validate()) {
                  try {
                    ...
                    Navigator.of(dialogContext).pop();
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                        content: Text('Funcionário atualizado!'),
                        backgroundColor: Colors.green));
                  } catch (e) {
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                        content: Text('Falha: ${e.toString()}'),
  ...
  void _deleteSelectedEmployees() {
    ...
    List<Employee> employeesToDelete = _selectedEmployeeIndices
        .map((index) {
          if (index < presenter.funcionarios.length)
            return presenter.funcionarios[index];
          return null;
        })
        .whereType<Employee>()
        .toList();
    ...
    if (employeesToDelete.isEmpty && _selectedEmployeeIndices.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Erro ao identificar funcionários.'),
          backgroundColor: Colors.red));
      return;
    }
    ...
    _confirmDeleteDialog(context, () async {
      ...
      for (var employee in employeesToDelete) {
        try {
          await presenter.deleteEmployee(
              id: employee.id); // Assuming deleteEmployee takes id
          successCount++;
        } catch (e) {
      ...
      if (successCount > 0 && successCount == employeesToDelete.length) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text('$successCount funcionário(s) excluído(s)!'),
            backgroundColor: Colors.green));
      } else if (successCount > 0) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(
                '$successCount funcionário(s) excluído(s). Alguns falharam.'),
            backgroundColor: Colors.orange));
      } else {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Falha ao excluir funcionário(s).'),
            backgroundColor: Colors.red));
      }
    }, _selectedEmployeeIndices.length.toString(), multiple: true);
  }
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text('Tabela de todos os seus funcionários cadastrados',
                        ...
                        children: [
                          ...
                          OutlinedButton.icon(
                            ...
                            onPressed: () => ScaffoldMessenger.of(context)
                                .showSnackBar(const SnackBar(
                                    content:
                                        Text('Exportar não implementado.'))),
                            style: OutlinedButton.styleFrom(
                                foregroundColor: const Color(0xFF4A5568),
                                side: BorderSide(color: Colors.grey.shade300),
                                padding: const EdgeInsets.symmetric(
                      ...
                      child: Consumer<EmployeesPresenter>(
                        builder: (context, presenter, child) {
                          if (presenter.isLoading &&
                              presenter.funcionarios.isEmpty) {
                            return const Center(
                          ...
                          final List<List<String>> tableData =
                              presenter.funcionarios.map((employee) {
                            // Assuming Employee model has a status or you derive it
                            String status = employee.id % 2 == 0
                                ? 'Active'
                          ...
                          return ResponsiveTable(
                            ...
                            onSelectionChanged: (selectedIndices) {
                              setState(() {
                                _selectedEmployeeIndices = selectedIndices;
                              });
                            },
                            onEdit: (rowIndex) {
                              if (rowIndex < presenter.funcionarios.length) {
                                _showEditEmployeeDialog(
                                    presenter.funcionarios[rowIndex]);
                              }
                            },
                            onDelete: (rowIndex) {
                              if (rowIndex < presenter.funcionarios.length) {
                                ...
                                _confirmDeleteDialog(context, () async {
                                  try {
                                    ...
                                    ScaffoldMessenger.of(context).showSnackBar(
                                        const SnackBar(
                                            content:
                                                Text('Funcionário excluído!'),
                                            backgroundColor: Colors.green));
                                  } catch (e) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                        SnackBar(
                                            content:
                                                Text('Falha: ${e.toString()}'),
                                            backgroundColor: Colors.red));
                                  }
                                }, employee.nome);
                              }
